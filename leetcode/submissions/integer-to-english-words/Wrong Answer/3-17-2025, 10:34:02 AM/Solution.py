...
class Solution:
    def __init__(self) -> None:
        ...
        self.tens = ["ERROR", "ERROR", "Twenty", "Thirty", "Fourty", "Fifty", "Sixty", "Seventy", "Eighty", "Ninety"]
    ...
    def numberToWords(self, num: int) -> str:
        ...
        ans = []
        for i in range(ceil(len(str(num))*1.0/3)): # ones to up
            thsd_str = ""
            thsd_num = num % 1000
...
# 23: Twenty Three
# 
# hundred -> if 0 then nothing, else x hundred
# tens -> if 0 then nothing, if 1 then SPECIAL, else MAP
# ones -> if ONLY 0 then zero, else nothing
#      -> if tens is 1 then ???