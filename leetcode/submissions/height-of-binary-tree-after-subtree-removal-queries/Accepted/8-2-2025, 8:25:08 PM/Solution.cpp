...
public:
    ...
    void dp(TreeNode* node, int height) {
        ...
        if (node->left) {
            memo[node->left->val-1] = max(height-1, memo[node->val-1]);
            if (node->right) 
        ...
        if (node->right) {
            memo[node->right->val-1] = max(height-1, memo[node->val-1]);
            if (node->left) 
                memo[node->right->val-1] = 
                    max(memo[node->right->val-1], depth[node->left->val-1] + height);
        }
        // if (node->left)
        //     std::cout << node->left->val - 1 << " l " << memo[node->left->val-1] << std::endl;
        // if (node->right)
        //     std::cout << node->right->val - 1 << " r " << memo[node->right->val-1] << std::endl;
        // std::cout << std::endl;
